(function(){
  'use strict';
  // Run only once in the top-level page. Running this bridge in every iframe
  // can create duplicate floating bars and null element handlers.
  ...
  function getAppState(){
    ...
    return {authenticated,schedule,campusStartDuration,online:navigator.onLine};
  }
  ...
  async function syncAppToExtension(force){
    ...
    try{
      const msg={type:'APP_STATE',authenticated:a.authenticated,schedule:a.schedule,campusStartDuration:a.campusStartDuration,online:navigator.onLine,audioBlobs:blobs,force:true};
  ...
  function mountBar(){
    ...
    chrome.runtime.onMessage.addListener((m)=>{
      if(!m) return;
      if(m.type==='EXT_STATE' || m.type==='EXT_STATE_TO_APP') render(m.state);
      if(m.type==='EXT_STATE_TO_APP' && location.href.startsWith(APP)){
        // Apply extension state into the Bell App immediately, without waiting
        // for a reload or another focus event.
        try{
          const st=m.state||{};
          if(Array.isArray(st.schedule) && st.schedule.length){
            localStorage.setItem(KEY,JSON.stringify(st.schedule));
            window.postMessage({source:'UMC_EXTENSION',type:'STATE_APPLIED',state:st},'*');
          }
          if(st.authenticated){
            localStorage.setItem(AUTH,'1');
            localStorage.setItem(GLOBAL_AUTH,'1');
          }
        }catch(e){}
      }
      if(m.type==='REQUEST_APP_SYNC' && location.href.startsWith(APP)){
        syncAppToExtension(true);
      }
    });
    function bind(id, fn){ const el=$(id); if(el) el.addEventListener('click',fn); }
    bind('app',()=>{ try{ const r=chrome.runtime.sendMessage({type:'OPEN_DASH'}); if(r&&typeof r.catch==='function') r.catch(()=>{}); }catch(e){} });
    bind('sync',async()=>{ try{ await chrome.runtime.sendMessage({type:'REQUEST_APP_SYNC'}); }catch(e){} try{ await chrome.runtime.sendMessage({type:'SYNC_NOW'}); }catch(e){} try{ await syncExtensionToApp(); }catch(e){} });
